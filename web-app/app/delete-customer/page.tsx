...
export default function DeleteCustomerPage() {
  // Typ auf any gesetzt, da CustomerID in der DB oft ein String (z.B. "ALFKI") ist
  const [customers, setCustomers] = useState<{ CustomerID: any; CustomerName: string }[]>([]);
  ...
  async function handleDelete(e: React.FormEvent) {
    ...
    // GEÄNDERT: Vergleich über String, damit auch Text-IDs (wie 'ALFKI') gefunden werden
    const customer = customers.find(c => String(c.CustomerID) === String(selectedId));
    ...
    setMsg("");
    try {
      const res = await fetch(`${API_BASE}/customers/${selectedId}`, { method: "DELETE" });
      if (!res.ok) {
        // Falls der Server einen Fehler sendet (z.B. wegen bestehender Bestellungen)
        const errData = await res.json().catch(() => ({}));
        throw new Error(errData.message || "Cannot delete Customer: Customer is linked to active or past orders. Please delete associated orders first or mark the customer as inactive.");
      }
      setMsg(`✅ Customer "${name}" deleted.`);
      // GEÄNDERT: Auch hier String-Vergleich für die UI-Aktualisierung
      setCustomers(prev => prev.filter(c => String(c.CustomerID) !== String(selectedId)));
      setSelectedId("");
    } catch (err: any) {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-white text-zinc-900">
      ...
      <main className="relative z-10 flex min-h-screen items-center justify-center px-6 py-12">
        <div className="panel w-full max-w-xl">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-xl font-bold tracking-tight">Delete Customer</h1>
              <p className="text-[10px] text-zinc-400 uppercase font-black mt-1 tracking-widest">Danger Zone</p>
            </div>
          ...
          <form onSubmit={handleDelete} className="grid gap-4">
            <div className="card-slim">
              <label className="field">
                ...
                <select className="input-slim" value={selectedId} onChange={(e) => setSelectedId(e.target.value)} disabled={loading || deleting} required>
                  <option value="">-- Please select --</option>
                  {customers.map(c => <option key={String(c.CustomerID)} value={String(c.CustomerID)}>{c.CustomerName}</option>)}
                </select>
              </label>
            </div>

            <div className="flex items-center gap-3 mt-4">
              <button className="delete-btn" type="submit" disabled={!selectedId || deleting}>
                {deleting ? "Processing..." : "Delete Customer"}
              </button>
      ...
      <style>{`
        ...
        .delete-btn{ padding: 12px 20px; border-radius: 12px; background: #111; color: #fff; font-weight: 900; font-size: 14px; border: none; cursor: pointer; transition: transform 0.15s; }
        .delete-btn:hover:not(:disabled){ transform: translateY(-1px); background: #ef4444; }
