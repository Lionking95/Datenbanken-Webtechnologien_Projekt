...
export default function UpdateCategoryPage() {
  const [categories, setCategories] = useState<{ CategoryID: number; CategoryName: string; Description?: string }[]>([]);
  ...
  useEffect(() => {
    if (selectedId) {
      const cat = categories.find(c => c.CategoryID === Number(selectedId));
      if (cat) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload: any = {};
    if (categoryName.trim() !== "") payload.CategoryName = categoryName;
    if (description.trim() !== "") payload.Description = description;

    try {
      const res = await fetch(`${API_BASE}/categories/${selectedId}`, {
        method: "PATCH",
      ...
      setCategories(prev => prev.map(c => 
        c.CategoryID === Number(selectedId) 
        ? { ...c, CategoryName: categoryName, Description: description } 
        : c
      ));
    } catch (err: any) {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-white text-zinc-900">
      ...
      <main className="relative z-10 flex min-h-screen items-center justify-center px-6 py-12">
        <div className="panel w-full max-w-xl">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-xl font-bold tracking-tight">Update Category</h1>
              <p className="text-[10px] text-zinc-400 uppercase font-black mt-1 tracking-widest">Edit category information</p>
            </div>
            <div className="flex gap-2">
              <Link className="ghost-btn-sm" href="/categories">⬅ Back</Link>
          ...
          <form onSubmit={handleSubmit} className="grid gap-4">
            <div className="card-slim">
              <label className="field">
                <span className="label-sm">Select Category</span>
            ...
            <div className="card-slim grid gap-4">
              <label className="field">
                <span className="label-sm">New Category Name</span>
                <input className="input-slim" placeholder="e.g. Beverages" value={categoryName} onChange={(e) => setCategoryName(e.target.value)} disabled={!selectedId || updating} />
              </label>

              <label className="field">
                <span className="label-sm">New Description</span>
                <input className="input-slim" placeholder="e.g. Soft drinks and coffees" value={description} onChange={(e) => setDescription(e.target.value)} disabled={!selectedId || updating} />
              </label>
            </div>

            <div className="flex items-center gap-3 mt-4">
              <button className="submit-btn" type="submit" disabled={!selectedId || updating}>{updating ? "Saving..." : "Save Changes"}</button>
              <Link className="cancel-link" href="/categories">Cancel</Link>
            </div>
      ...
      <style>{`
        ...
        .submit-btn{ padding: 12px 20px; border-radius: 12px; background: #111; color: #fff; font-weight: 900; font-size: 14px; border: none; cursor: pointer; transition: transform 0.15s, background 0.2s; }
