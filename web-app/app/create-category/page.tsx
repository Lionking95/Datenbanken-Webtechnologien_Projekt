...
const COPIES_PER_IMAGE = 6; // Synced: 48 Bilder insgesamt
...
function readTextSafely(res: Response) {
...
export default function CreateCategoryPage() {
  ...
  // Exact Background Animation Logic from Home
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setMsg(`❌ ${res.status}: ${data?.error || raw || "POST failed"}`);
        return;
      }
      const insertId = data?.insertId;
      setMsg(`✅ Category created successfully${insertId ? ` (ID: ${insertId})` : ""}`);
      setCategoryName("");
      setDescription("");
    } catch (err: any) {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-white text-zinc-900">
      {/* Animated Background (Exact Copy from Home) */}
      ...
      <main className="relative z-10 flex min-h-screen items-center justify-center px-6 py-12">
        <div className="panel w-full max-w-xl">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-xl font-bold tracking-tight">Create Category</h1>
              <p className="text-[10px] text-zinc-400 uppercase font-black mt-1 tracking-widest">Add new category</p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="grid gap-4">
            ...
            <div className="card-slim">
              <label className="field">
                ...
                <input
                  className="input-slim"
                  placeholder="e.g. Soft drinks, coffees, and teas"
                  value={Description}
      ...
      <style>{`
        ...
        .label-sm{ font-size: 10px; font-weight: 900; color: #999; text-transform: uppercase; display: block; margin-bottom: 4px; }
