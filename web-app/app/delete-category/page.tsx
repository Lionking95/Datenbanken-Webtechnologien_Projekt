...
export default function DeleteCategoryPage() {
  ...
  async function handleDelete(e: React.FormEvent) {
    ...
    const categoryName = categories.find(c => c.CategoryID === Number(selectedId))?.CategoryName;
    if (!confirm(`Are you sure you want to delete "${categoryName}"?`)) return;

    setDeleting(true);
    setMsg("");
    try {
      const res = await fetch(`${API_BASE}/categories/${selectedId}`, { method: "DELETE" });
      if (!res.ok) throw new Error("Cannot delete category: This category is currently linked to existing products.");
      setMsg(`✅ Category "${categoryName}" deleted.`);
      setCategories(prev => prev.filter(c => c.CategoryID !== Number(selectedId)));
      setSelectedId("");
    } catch (err: any) {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-white text-zinc-900">
      {/* Background Sprites - Identical to Product Page */}
      ...
      <main className="relative z-10 flex min-h-screen items-center justify-center px-6 py-12">
        <div className="panel w-full max-w-xl">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-xl font-bold tracking-tight">Delete Category</h1>
          ...
          <form onSubmit={handleDelete} className="grid gap-4">
            ...
            {msg && (
              <div className={`status-msg ${msg.startsWith("✅") ? "ok" : "err"}`}>{msg}</div>
            )}
          </form>
        </div>
      </main>
      ...
      <style>{`
        ...
        .status-msg{ border-radius: 12px; padding: 10px; font-weight: 800; font-size: 13px; }
