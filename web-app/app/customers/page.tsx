...
const COPIES_PER_IMAGE = 6; // Erhöht auf 48 Bilder für maximale Konsistenz
        ...
        <div className="panel w-full max-w-5xl">
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
            ...
            <div className="flex gap-2">
              ...
              <Link className="ghost-btn-sm" href="/create-customer">➕ Create</Link>
              <Link className="ghost-btn-sm" href="/update-customer">✏️ Update</Link>
              <Link className="ghost-btn-sm" href="/delete-customer">🗑️ Delete</Link>
            </div>
          </div>

          <div className="card-slim mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          {loading && <div className="text-center py-10 font-black text-zinc-300 animate-pulse uppercase tracking-widest">Loading Client Data...</div>}
          ...
          {!loading && (
            <div className="overflow-hidden border border-zinc-100 rounded-2xl bg-white/50 backdrop-blur-sm">
              <table className="w-full text-left text-sm border-collapse">
                <thead>
                  <tr className="bg-zinc-50/50">
                    ...
                    <th className="p-4 font-black uppercase text-[10px] text-zinc-400 tracking-wider border-b border-zinc-100">Customer Name</th>
                    <th className="p-4 font-black uppercase text-[10px] text-zinc-400 tracking-wider border-b border-zinc-100">Contact</th>
                    <th className="p-4 font-black uppercase text-[10px] text-zinc-400 tracking-wider border-b border-zinc-100">Location</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-50">
                  {visibleCustomers.map((c) => (
                    <tr key={c.CustomerID} className="hover:bg-zinc-50/80 transition-colors">
                      <td className="p-4 font-mono font-bold text-zinc-400">#{c.CustomerID}</td>
                      <td className="p-4 font-bold text-zinc-900">{c.CustomerName}</td>
                      <td className="p-4 text-zinc-600">{c.ContactName || "—"}</td>
                      <td className="p-4">
                        <span className="text-zinc-900 font-medium">{c.City || "—"}</span>
                        <span className="text-zinc-400 text-xs ml-2 uppercase font-bold">{c.Country}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              {visibleCustomers.length === 0 && (
                <div className="p-12 text-center font-bold text-zinc-300 uppercase tracking-tighter">No customers found.</div>
              )}
            </div>
          )}
        </div>
