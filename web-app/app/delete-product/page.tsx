...
export default function DeleteProductPage() {
  ...
  async function handleDelete(e: React.FormEvent) {
    ...
    // VERBESSERTER VERGLEICH: Wandelt IDs in Strings um und entfernt Leerzeichen
    const product = products.find(p => String(p.ProductID).trim() === String(selectedId).trim());
    ...
    setMsg("");
    try {
      const res = await fetch(`${API_BASE}/products/${selectedId}`, { method: "DELETE" });
      
      if (!res.ok) {
        const errorData = await res.json().catch(() => ({}));
        throw new Error(errorData.error || "Cannot delete product: Product is part of active or past transactions. Remove associated order entries first.");
      }

      setMsg(`✅ Product "${name}" deleted.`);
      
      // VERBESSERTER FILTER: Auch hier String-Konvertierung für zuverlässiges UI-Update
      setProducts(prev => prev.filter(p => String(p.ProductID).trim() !== String(selectedId).trim()));
      setSelectedId("");
    } catch (err: any) {
      ...
      <main className="relative z-10 flex min-h-screen items-center justify-center px-6 py-12">
        <div className="panel w-full max-w-xl">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-xl font-bold tracking-tight">Delete Product</h1>
          ...
          <form onSubmit={handleDelete} className="grid gap-4">
            <div className="card-slim">
              <label className="field">
                ...
                >
                  ...
                  {products.map(p => (
                    <option key={p.ProductID.toString()} value={p.ProductID.toString()}>
            ...
            <div className="flex items-center gap-3 mt-4">
              <button className="delete-btn" type="submit" disabled={!selectedId || deleting}>
                {deleting ? "Processing..." : "Delete Product"}
              </button>
