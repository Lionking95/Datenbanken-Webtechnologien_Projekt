...
export default function CreateProductPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (!sRes.ok || !cRes.ok) throw new Error("Server response not OK");
        const sData = await sRes.json();
        const cData = await cRes.json();
        setSuppliers(Array.isArray(sData) ? sData : []);
        setCategories(Array.isArray(cData) ? cData : []);
      } catch (err) {
        setMsg("❌ Error loading reference data");
      } finally {
        setLoading(false);
      }
    };
  ...
  // Exact Background Animation Logic
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setProductName(""); setPrice(""); setSupplierID(""); setCategoryID(""); setUnit("");
    } catch (err: any) {
        ...
        <div className="panel w-full max-w-xl">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-[10px] text-zinc-400 uppercase font-black mt-1 tracking-widest">Add new product</p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="grid gap-4">
            <div className="card-slim">
              <label className="field">
                <span className="label-sm">Product Name *</span>
                <input 
            ...
            <div className="card-slim grid gap-4 sm:grid-cols-2">
              <label className="field">
            ...
            <div className="card-slim grid gap-4">
              <label className="field">
                <span className="label-sm">Supplier</span>
                <select 
                  className="input-slim"
                  value={SupplierID}
                ...
                >
                  ...
                  {suppliers.map(s => (
                    <option key={s.SupplierID} value={s.SupplierID}>
                      {s.SupplierName || s.suppliername || "Unnamed Supplier"}
                    </option>
                  ))}
                </select>
              </label>
                ...
                >
                  ...
                  {categories.map(c => (
                    <option key={c.CategoryID} value={c.CategoryID}>
                      {c.CategoryName || c.categoryname || "Unnamed Category"}
                    </option>
                  ))}
                </select>
              </label>
